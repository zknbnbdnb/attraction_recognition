...
def train(train_loader, model, criterion, optimizer, epoch, writer):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        [prec1, prec5], class_to = accuracy(output, target, topk=(1, 2))
...
if __name__ == "__main__":
    # -------------------------------------------- step 1/4 : 加载数据 ---------------------------
    train_dir_list = 'D:\pytorch\Dataset/train.txt'
    valid_dir_list = 'D:\pytorch\Dataset/val.txt'
    batch_size = 16
    epochs = 10
    num_classes = 5
    train_data = Garbage_Loader(train_dir_list, train_flag=True)
    valid_data = Garbage_Loader(valid_dir_list, train_flag=False)
    train_loader = DataLoader(dataset=train_data, num_workers=0,
                              pin_memory=True, batch_size=batch_size, shuffle=True)
    valid_loader = DataLoader(
        dataset=valid_data, num_workers=0, pin_memory=True, batch_size=batch_size)
    ...
    scheduler = torch.optim.lr_scheduler.StepLR(
        optimizer, step_size=lr_stepsize, gamma=0.1)
    ...
    for epoch in range(epochs):
        ...
        # 在验证集上测试效果
        valid_prec1, valid_prec5 = validate(
            valid_loader, model, criterion, epoch, writer, phase="VAL")
